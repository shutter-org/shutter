  ...
  <div class="fixed inset-0 z-40 flex justify-center items-center gap-5">
    ...
    >
      ...
      <div class="flex gap-1 flex-col w-full">
        <div
          v-if="!login"
          class="rounded-md items-center flex flex-col relative"
        >
          <div class="w-full rounded-md h-10 input-color px-4">
            <input
              class="z-2 outline-none input-color border-0 box-border h-10 w-full"
              placeholder="Email"
              v-model="email"
            />
          </div>
        ...
        <div class="rounded-md this">
          <div class="w-full rounded-md h-10 input-color px-4">
            <input
              class="z-2 outline-none input-color border-0 box-border h-10 w-full hidePassword"
      ...
      <button
        v-if="login"
        @click="SignIn"
        class="bg-gray-100 special-text-color rounded-md h-9 w-36"
      >
        Log in
      </button>
      <button
      ...
      <div
        class="border-t no-account w-full flex justify-center items-center space-x-2"
      >
        <span v-if="login">Don't have an account?</span>
        <button
          v-if="login"
          @click="switchView"
          class="special-text-color text-lg"
        >
          Signup
        </button>
