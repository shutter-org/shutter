...
export const usePublicationStore = defineStore("publication", () => {
  ...
  async function loadMorePublications(page: number) {
    if (
      ...
      homePublications.value.length < page * serverPageQte.value
    ) {
  ...
  async function loadShownPublication(publicationId: string) {
    ...
    if (res.status !== 200) {
      ...
      lastShownPublications.value.set(
        shownPublication.publication_id,
        shownPublication
      );
      isBusy.value = false;
  ...
  async function modifyShownPublication(
      ...
      if (res.status === 201) {
        if (
          ...
          (pub.comments.length === pub.nb_comments &&
            Math.floor(pub.nb_comments / serverPageQte.value) !== 0)
        ) {
          const data = await res.json();
          const newCom = {
